...
public class DAOUsuarioRepository {
	...
	public ModelLogin gravaUsuario(ModelLogin objeto, Long userLogado) throws SQLException {
		...
		if (objeto.isNovo()) {
			...
			preparedSql.setDate(13, new Date(objeto.getDataNascimento().getTime()));
	...
	public List<ModelLogin> consultausuarioListPaginado(Long userLogado, Integer offset) throws Exception {
		...
		String sql = "select * from model_login where useradmin is false and usuario_id = ? order by nome offset "
				+ offset + " limit 5";
		PreparedStatement statement = connection.prepareStatement(sql);
	...
	public List<ModelLogin> consultausuarioListOffset(String nome, Long userLogado, Integer offset) throws Exception {
		...
		String sql = "select * from model_login where upper(nome) like upper(?) and useradmin is false and usuario_id = ? order by nome offset " + offset + " limit 5";
		PreparedStatement statement = connection.prepareStatement(sql);
		statement.setString(1, "%" + nome + "%");
		statement.setLong(2, userLogado);
